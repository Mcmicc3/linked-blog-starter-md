# Chapter 1.  Introduction to Red Hat Enterprise Linux

Fedora → CentOS Stream → RHEL

**Fedora asks:**

> “What should the future of Linux look like?”

**CentOS Stream asks:**

> “What should the next RHEL look like?”

**RHEL asks:**

> “What can we commit to supporting for enterprise customers?



![[Pasted image 20260812161106.png]]

Two general classes of open source license are particularly important:

- _Copyleft_ licenses are designed to encourage keeping the code open source.
    
- _Permissive_ licenses are designed to maximize code reusability.

![[Pasted image 20260812161156.png]]

#### Red Hat Universal Base Image
UBI is designed to be a foundation *for cloud-native and web application use cases that are developed in containers*. All UBI content is a subset of RHEL, with packages sourced from secure RHEL channels, and UBI is supported similar to RHEL when run on Red Hat-supported platforms such as OpenShift and RHEL hosts.

#### RHEL-based Image Optimized for Edge
RHEL provides the ability to create purpose-built operating system images with Red Hat Enterprise Linux *image builder.* With this mechanism, IT teams can build, deploy, and maintain these RHEL images in less time over the life of the system. Image-based deployments are optimized for various edge architectures, and are customizable for specific edge deployments.

#### Red Hat Enterprise Linux CoreOS
Red Hat Enterprise Linux CoreOS (RHCOS) is not a stand-alone operating system. CoreOS is *built from RHEL components, and is then released, upgraded, and managed as part of the Red Hat OpenShift Container Platform (RHOCP) for cloud-native applications.* RHCOS is fundamentally an *image-based RHEL container host*, which uses the Container Runtime Interface (CRI-O)-compliant container engine that is integrated in RHOCP. To learn more about Red Hat Enterprise Linux CoreOS, begin by becoming familiar with OpenShift and containers.

#### Red Hat Enterprise Linux Continuous Development
![[Pasted image 20260812162343.png]]

---

# Chapter 2.  Accessing the Command Line

#### Virtual Consoles
Each virtual console supports an independent login session. You can switch between the virtual consoles by pressing **Ctrl**+**Alt** and a function key (**F1** through **F6**) at the same time. Most of these virtual consoles run a terminal that provides a text login prompt. If you enter your username and password correctly, then you log in and get a shell prompt.

#### Public Key Authentication
In the next example, a user with a shell prompt on the `host` machine uses the public key authentication method to log in to the `remotehost` remote system as the `remoteuser` user. The `ssh` command `-i` option specifies the user's private key file, which is `mylab.pem`. The matching public key is already set up as an authorized key in the `remoteuser` account.

``` bash
user@host:~$ ssh -i mylab.pem remoteuser@remotehost
remoteuser@remotehost:~$
```
---

##### Commands
whoami
head
tail
wc

"*Tab Completion*"

#### Command Separators
To type more than one command on a single line, use the semicolon (**;**) as a command separator. A semicolon is a member of a class of characters called _metacharacters_ that have a special interpretation for Bash. In this case, the output of both commands is displayed before the following shell prompt appears.

``` bash
user@host:~$ command1 ; command2
command1 output
command2 output
```

##### Date command
``` bash
user@host:~$ date
Tue Apr 29 17:58:22 UTC 2025
user@host:~$ date +%R
17:58
user@host:~$ date +%x
04/29/25
```

cat
passwd
file
* Linux does not require file name extensions to classify files by type. The `file` command scans the compiled header of a file for a 2-digit `magic number` and displays its type. Text files are recognized because they are not compiled.
tail -n 5 {argument}
wc

***TIP:*** type a command with --, then hit tab twice to see all the options!

Using `\` as a method for typing a long command through multiple lines 

Using `!` to execute the previous command
* The exclamation point character (`!`) is a metacharacter to expand previous commands without retyping them. The `!number` command expands to the command that matches the specified number. The `!string` command expands to the most recent command that begins with the specified string.
* ***TIP***: Use Alt + s, to make the current or previous command sudo

###### heh? 
Use either the **Esc**+**.** or **Alt**+**.** key combination simultaneously to insert the last word of the previous command at the cursor's current location. The repeated use of the key combination replaces that text with the last word of earlier commands in history. The **Alt**+**.** key combination is particularly convenient, because you can hold down **Alt** and press **.** repeatedly to quickly cycle earlier commands.
* Bash shortcut or command copies the last argument of previous commands

#### Edit the command line

|Shortcut|Description|
|:--|:--|
|**Ctrl**+**A**|Jump to the beginning of the command line.|
|**Ctrl**+**E**|Jump to the end of the command line.|
|**Ctrl**+**U**|Clear from the cursor to the beginning of the command line.|
|**Ctrl**+**K**|Clear from the cursor to the end of the command line.|
|**Ctrl**+**LeftArrow**|Jump to the beginning of the previous word on the command line.|
|**Ctrl**+**RightArrow**|Jump to the end of the next word on the command line.|
|**Ctrl**+**R**|Search the history list of commands for a pattern.|

---


# Chapter 3. Getting Help from Local Documentation

### Useful
``` bash
man -f w
```
Produces a single line of what the command is

**Also goated**
``` bash
man -k {anything}
man -k FQDN
man -k selinux
```
What it does, is it returns everytime a specific keyword is found anywhere in the man pages. Sometimes this leads to useful commands you've never heard of before.


**3.1 Common Sections of the Linux Manual**

|Section|Content type|Description|
|:--|:--|:--|
|1|User commands|Both executable and shell programs|
|2|System calls|Kernel routines that are invoked from user space|
|3|Library functions|Provided by program libraries|
|4|Special files|For example, device files|
|5|File formats|For many configuration files and structures|
|6|Games and screensavers|Historical section for amusing programs|
|7|Conventions, standards, and miscellaneous|Protocols and file systems|
|8|System administration and privileged commands|Maintenance tasks|
|9|Linux kernel API|Internal kernel calls|

**3.2 Headings**

| Heading     | Description                                                           |
| :---------- | :-------------------------------------------------------------------- |
| NAME        | Subject name, usually a command or file name, with a brief definition |
| SYNOPSIS    | Summary of the command syntax                                         |
| DESCRIPTION | Description to provide a basic understanding of the topic             |
| OPTIONS     | Explanation of the command execution options                          |
| EXAMPLES    | Examples of how to use the command, function, or file                 |
| FILES       | A list of files and directories that are related to the man page      |
| SEE ALSO    | Related information, typically other man page topics                  |
| BUGS        | Known bugs in the software                                            |
| AUTHOR      | Information about who contributed to the development of the topic     |

**3.3 Navigate Man Pages**

| Command         | Result                                                    |
| :-------------- | :-------------------------------------------------------- |
| **Spacebar**    | Scroll forward (down) one screen.                         |
| **PageDown**    | Scroll forward one screen.                                |
| **PageUp**      | Scroll backward (up) one screen.                          |
| **DownArrow**   | Scroll forward one line.                                  |
| **UpArrow**     | Scroll backward one line.                                 |
| **D**           | Scroll forward one half-screen.                           |
| **U**           | Scroll backward one half-screen.                          |
| ``/_`string`_`` | Search forward for _string_ in the man page.              |
| **N**           | Repeat previous search forward in the man page.           |
| **Shift**+**N** | Repeat previous search backward in the man page.          |
| **G**           | Go to the start of the man page.                          |
| **Shift**+**G** | Go to the end of the man page.                            |
| **Q**           | Exit the man page and return to the command shell prompt. |

**Searching for Man Pages by Keyword**
To search for manual pages by their title, use the `man -f` command. This is identical to using the `whatis` command, which displays a one-line summary for each matching page.
``` bash
man -f passwd
passwd (1)           - update users authentication tokens
passwd (1ossl)       - OpenSSL application commands
passwd (5)           - password file
```

For a broader search, use the `man -k` command (equivalent to the `apropos` command). This command is a more powerful alternative that searches not only the titles but also the descriptions of all manual pages for your keyword, helping you find relevant pages even if you do not know the exact name. Your keywords are treated as regular expressions for pattern matching.

``` bash
user@host:~$ man -k passwd
chgpasswd (8)        - update group passwords in batch mode
chpasswd (8)         - update passwords in batch mode
fgetpwent_r (3)      - get passwd file entry reentrantly
getpwent_r (3)       - get passwd file entry reentrantly
gpasswd (1)          - administer /etc/group and /etc/gshadow
_...output omitted..._
```

The `man` command `-K` (uppercase) option searches for the keyword in the full-text page, not only in the titles and descriptions. A full-text search uses greater system resources and takes more time.

With the full-text page search, the `man` command displays the first page with a match. Press **Q** to exit this first page, and the `man` command displays the next page.

In this example, the `man` command displays each match, and you can view or skip each one.

``` bash
user@host:~# **`man -K passwd`**
_...output omitted..._
--Man-- next: cut(1p) [ view (return) | skip (Ctrl-D) | quit (Ctrl-C) ]
**`Ctrl-D`**
--Man-- next: logname(1p) [ view (return) | skip (Ctrl-D) | quit (Ctrl-C) ]
**`Ctrl-D`**
--Man-- next: sort(1p) [ view (return) | skip (Ctrl-D) | quit (Ctrl-C) ]
**`Ctrl-D`**
--Man-- next: xargs(1) [ view (return) | skip (Ctrl-D) | quit (Ctrl-C) ]
**`Ctrl-D`**
--Man-- next: chage(1) [ view (return) | skip (Ctrl-D) | quit (Ctrl-C) ]
**`Ctrl-C`**
```



# Chapter 4.  Registering Systems for Red Hat Support

It's common to connect new Red Hat clients to the CDN. Red Hat CDN comes with *content*: **security updates**, **bugfixes**, **improvements**; *Lightspeed*: Analytics, Command Line Assistant.

Many ways to connect to Red Hat:
##### rhc
https://console.redhat.com
Modern Solution
System is tied into three things, content network, inside analytics, and remote management. This creates a confirmation that everything is working. 
##### subscription-manager
**Long standing tool**. Focuses on attaching subscription, and enabling repositories. Many automation scripts, legacy workflows, and anaconda, more options, all of these use subscription manager under the hood. 
This is the service that regularly analyze the system

##### Red Hat satellite
Remote management, policy enforcement. Acts as the single

*Entitlements*: Lists your rights to specific software you should have access to.


### Registering for Red Hat (rhc Example)
![[Pasted image 20260902182810.png]]
``` bash
ls -l /etc/motd.d/insights-client
ssh servera
sudo -i  `#changes to root`
rhc status
rhc connect --username ricadodacosta
```
![[Pasted image 20260902183033.png]]
*Example of not connected*

rhc is only available for 8.8 and later.

**Optional**
``` bash
root@host:~# **`rhc connect --disable-feature=analytics,remote-management`**
```
Keeps content repositories, but excludes analytics and remote-management

### Registering through subscription-manager 
``` bash
subscription-manager register --username ricardodacosta
subscription-manager identity
subscription-manager status
insights-client --register
```
![[Pasted image 20260902183310.png]]
*Shows more information*

![[Pasted image 20260902183510.png]]
![[Pasted image 20260902183618.png]]*Tells you if you're registered, but doesn't tell you how*
![[Pasted image 20260902183714.png]]

##### Setting System Purpose Attributes
_System purpose_ is an optional tagging mechanism that helps to filter your inventory. You can set the following attributes and values by using the `subscription-manager` command:
**Role**
- Red Hat Enterprise Linux Server
    
- Red Hat Enterprise Linux Workstation
    
- Red Hat Enterprise Linux Compute Node

**Service-Level Agreement**
- Premium
    
- Standard
    
- Self-Support
    
**Usage**
- Production
    
- Development/Test
    
- Disaster Recovery

``` bash
root@host:~# **`subscription-manager syspurpose role \ --set 'Red Hat Enterprise Linux Workstation'`**
role set to "Red Hat Enterprise Linux Workstation".

root@host:~# **`subscription-manager syspurpose service-level \ --set 'Premium'`**
service_level_agreement set to "Premium".

root@host:~# **`subscription-manager syspurpose usage \ --set 'Development/Test'`**
usage set to "Development/Test".

root@host:~# **`subscription-manager syspurpose usage --list`**
+-------------------------------------------+
               Available usage
+-------------------------------------------+
 - Development/Test
```

### Inspecting Identity Certificates

A registered system stores several certificates under the */etc/pki* directory.

**The /etc/pki/product-default certificates**
Identify the Red Hat products that are installed on the system.

**The /etc/pki/consumer certificate and key pair**
Identifies the system.

**The /etc/pki/entitlement certificate and key pair**
Authenticates the system to repositories.

``` bash
root@host:~# **`rct cat-cert /etc/pki/product-default/479.pem`**

+-------------------------------------------+
	Product Certificate
+-------------------------------------------+

Certificate:
	Path: /etc/pki/product-default/479.pem
	Version: 1.0
	Serial: 271555466504741450059712551000979178078433728276
	Start Date: 2024-08-15 06:16:49+00:00
	End Date: 2044-08-15 06:16:49+00:00

_...output omitted..._

Product:
	ID: 479
	Name: Red Hat Enterprise Linux for x86_64
	Version: 10.0
	Arch: x86_64
	Tags: rhel-10,rhel-10-x86_64
	Brand Type:
	Brand Name:
```

# Chapter 5.  Getting AI-assisted Help with Red Hat Enterprise Linux Lightspeed

``` bash
root@host:~# dnf install command-line-assistant
user@host:~$ c chat "How do I list running services?"
user@host:~$ c chat "How do I check the status of the sshd service?"

*** Interactive Mode ***
user@host:~$ **`c chat --interactive`**
Welcome to the interactive mode for command line assistant! To exit, press Ctrl + C or type '.exit'.
>>>

*** Analyzing Command Output ***
user@host:~$ **`sudo journalctl -u httpd.service --since "1 hour ago" | \ c chat "What could be causing errors in these logs?"`**

*** Analyzing File Content ***
**user@host:~$ cat /var/log/messages | \ c chat "Look for critical errors in this log file."

*** Analyzing Data ***

user@host:~$ c chat -a /var/log/boot.log "Why did the last boot take so long?"

```

![[Pasted image 20260902204715.png]]

# Chapter 6

![[Pasted image 20260916211839.png]]

|Location|Purpose|
|:--|:--|
|`/boot`|Files to start the boot process.|
|`/dev`|Special device files that the system uses to access hardware.|
|`/etc`|System-specific configuration files.|
|`/home`|Home directory, where regular users store their data and configuration files.|
|`/root`|Home directory for the administrative superuser, `root`.|
|`/run`|Runtime data for processes that started since the last boot. This data includes process ID files and lock files. The contents of this directory are re-created on reboot. This directory consolidates the `/var/run` and `/var/lock` directories from earlier versions of Red Hat Enterprise Linux.|
|`/tmp`|A world-writable space for temporary files. Files that are not accessed, changed, or modified for 10 days are deleted from this directory automatically. The `/var/tmp` directory is also a temporary directory, in which files that are not accessed, changed, or modified in more than 30 days are deleted automatically.|
|`/usr`|Installed software, shared libraries, including files, and read-only program data. Significant subdirectories in the `/usr` directory include the following commands:<br><br>- `/usr/bin`: User commands<br>    <br>- `/usr/sbin`: System administration commands<br>    <br>- `/usr/local`: Locally customized software|
|`/var`|System-specific variable data should persist between boots. Files that dynamically change, such as databases, cache directories, log files, printer-spooled documents, and website content, might be found under the `/var` directory.|
*relative path versus absolute path*

***Interesting Tip***: 
``` bash
cd -
```
*returns your shell prompt to the working directory you were using immediately before the current working directory*

# Chapter 7.  Managing Files from the Command Line

Remember, you can make multiple directories with mkdir.
``` bash
mkdir ProjectX ProjectY ProjectZ
mkdir -p Thesis/Chapter1 Thesis/Chapter2 Thesis/Chapter3
ls -R Thesis/
```
*Use -p to make subdirectories.*
*Use -R to recursively show what's inside a directory.*

``` bash
user@host:~/ProjectY$ cp -r ../Thesis/ .
user@host:~/ProjectY$ ls -lR
.:
total 0
drwxr-xr-x. 5 user user 54 Mar  7 15:08 Thesis

./Thesis:
total 0
drwxr-xr-x. 2 user user 6 Mar  7 15:08 Chapter1
drwxr-xr-x. 2 user user 6 Mar  7 15:08 Chapter2
drwxr-xr-x. 2 user user 6 Mar  7 15:08 Chapter3

./Thesis/Chapter1:
total 0

./Thesis/Chapter2:
total 0

./Thesis/Chapter3:
total 0
```
*Note ls -lR and cp -r*
You can copy directories and their contents by using the `cp` command with `-r` option. Keep in mind that you can use the `.` and `..` special directories in command combinations. In the following example, the `Thesis` directory and its contents are copied to the `ProjectY` directory:

``` bash
rm -r Thesis/Chapter1
```
*The -r option removes directories.*
*You can include a -i option, so that it verifies with you first if you want to delete something.*
**rmdir only works with empty directories.**

## Creating Links Between Files

#### *Hard links*
Every file starts with a single hard link, from its initial name to the data on the file system. When you create a hard link to a file, you create another name that points to that same data.
``` bash
user@host:~$ **`pwd`**
/home/user
user@host:~$ **`ls -l newfile.txt`**
-rw-r--r--. `1` user user 0 May 30 19:19 newfile.txt
```
**The *1* in the last line relates to the number of hard links**

***ln command is used to create new hard links***
``` bash
user@host:~$ **`ln newfile.txt /tmp/newfile-hlink2.txt`**
user@host:~$ **`ls -l newfile.txt /tmp/newfile-hlink2.txt`**
-rw-rw-r--. `2` user user 12 May 30 19:19 /tmp/newfile-hlink2.txt
-rw-rw-r--. `2` user user 12 May 30 19:19 newfile.txt
```

***The ln -i command lists the files inode number***
* **Hard Linked files have the same inode**

``` bash
user@host:~$ **`ls -il newfile.txt /tmp/newfile-hlink2.txt`**
`8924107` -rw-rw-r--. 2 user user 12 May 30 19:19 /tmp/newfile-hlink2.txt
`8924107` -rw-rw-r--. 2 user user 12 May 30 19:19 newfile.txt
```
* **Hard links that reference the same file share the inode structure with the link count, access permissions, user and group ownership, time stamps, and file content.**

* *When that information is changed for one hard link, the other hard links for the same file also show the new information.* This behavior is because each hard link points to the same data on the storage device.

If the original file is deleted, you can still access the contents of the file provided that at least one other hard link exists. Data is deleted from storage only when the last hard link is deleted, which makes the file contents unreferenced by any hard link.

``` bash
user@host:~$ **`rm -f newfile.txt`**
user@host:~$ **`ls -l /tmp/newfile-hlink2.txt`**
-rw-rw-r--. `1` user user 12 Mar 30 19:19 /tmp/newfile-hlink2.txt
user@host:~$ **`cat /tmp/newfile-hlink2.txt`**
Hello World
```

#### Limitations of Hard Links
1. Hard links *only work with regular files*. You cannot use the `ln` command to create a hard link to a directory or a special file.
   
2. You can use hard links *only if both files are on the same* _file system_. The file-system hierarchy can include multiple storage devices. Depending on the configuration of your system, when you change into a new directory, that directory and its contents might be stored on a different file system.

**Use the ***df*** **command to see all mounted file systems, and their mount points.**

#### *Soft Links*
The `ln` command `-s` option creates a symbolic link, which is also called a _soft link_. A symbolic link is not a regular file. It is a special type of file that points to an existing file or directory.