...
```shell-session
Fr1zzleFry@htb[/htb]$ nmap <scan types> <options> <target>
```
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap -sS localhost
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.0/24 -sn -oA tnet | grep for | cut -d" " -f5
...
```shell-session
Fr1zzleFry@htb[/htb]$ cat hosts.lst
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap -sn -oA tnet -iL hosts.lst | grep for | cut -d" " -f5
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap -sn -oA tnet 10.129.2.18 10.129.2.19 10.129.2.20| grep for | cut -d" " -f5
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap -sn -oA tnet 10.129.2.18-20| grep for | cut -d" " -f5
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.18 -sn -oA host 
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.18 -sn -oA host -PE --packet-trace 
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.18 -sn -oA host -PE --reason 
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.18 -sn -oA host -PE --packet-trace --disable-arp-ping 
...
1. Based on the last result, find out which operating system it belongs to. Submit the name of the operating system as result.
		1. Apparently you can learn what type of OS a system is based on the TTL value
		2. Value of 128 = Windows, Value of 64 is Linux
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 --top-ports=10 
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p 21 --packet-trace -Pn -n --disable-arp-ping
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p 443 --packet-trace --disable-arp-ping -Pn -n --reason -sT 
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p 139 --packet-trace -n --disable-arp-ping -Pn
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p 445 --packet-trace -n --disable-arp-ping -Pn
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -F -sU
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -sU -Pn -n --disable-arp-ping --packet-trace -p 137 --reason 
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -sU -Pn -n --disable-arp-ping --packet-trace -p 100 --reason 
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -sU -Pn -n --disable-arp-ping --packet-trace -p 138 --reason 
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -Pn -n --disable-arp-ping --packet-trace -p 445 --reason  -sV
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p- -oA target
...
```shell-session
Fr1zzleFry@htb[/htb]$ cat target.nmap
...
```shell-session
Fr1zzleFry@htb[/htb]$ cat target.gnmap
...
```shell-session
Fr1zzleFry@htb[/htb]$ cat target.xml
...
```shell-session
Fr1zzleFry@htb[/htb]$ xsltproc target.xml -o target.html
```
...
1. Perform a full TCP port scan on your target and create an HTML report. Submit the number of the highest port as the answer.
		1. sudo nmap 10.129.2.49 -p- -oX target
		2. xsltproc target.xml -o target.html
		3. Open the html file in a web browser
		4. ????
		5. sudo nmap 10.129.2.49 -p- -oX target -T 5
		6. Success (**Way faster**)
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV
```
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV --stats-every=5s
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV -v 
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV -Pn -n --disable-arp-ping --packet-trace
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo tcpdump -i eth0 host 10.10.14.2 and 10.129.2.28
...
```shell-session
Fr1zzleFry@htb[/htb]$  nc -nv 10.129.2.28 25
...
1. Enumerate all ports and their services. One of the services contains the flag you have to submit as the answer.
	1. nmap 10.129.98.229 -p- -sV -T 5 
	2. Find all open ports (none of them are it)
	3. Open another terminal
	4. sudo tcpdump -i tun0 10.10.14.2 and 10.129.98.229
	5. nc -nv 10.129.98.229 80 (dumb luck)
	6. Let it run, stop the tcpdump terminal ^c
	7. Find the answer on the terminal running nc
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap <target> -sC
```

#### Specific Scripts Category

```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap <target> --script <category>
```

#### Defined Scripts

```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap <target> --script <script-name>,<script-name>,...
```
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p 25 --script banner,smtp-commands
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p 80 -A
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p 80 -sV --script vuln 
...
1. Use NSE and its scripts to find the flag that one of the services contain and submit it as the answer.
	1.  sudo nmap 10.129.98.229 -p- -T 5 -A --script discovery,version
	2. Scroll through results, discover /robots.txt on port 80
	3. Open browser, visit 10.129.98.229/robots.txt
	4. Success
	5. **Some flags and or scripts may have been unnecessary**
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.0/24 -F
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.0/24 -F --initial-rtt-timeout 50ms --max-rtt-timeout 100ms

<SNIP>
Nmap done: 256 IP addresses (8 hosts up) scanned in 12.29 seconds
```

**Notice the shorter duration of the scan**
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.0/24 -F | grep "/tcp" | wc -l
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.0/24 -F --max-retries 0 | grep "/tcp" | wc -l
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.0/24 -F -oN tnet.default
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.0/24 -F -oN tnet.minrate300 --min-rate 300
...
```shell-session
Fr1zzleFry@htb[/htb]$ cat tnet.default | grep "/tcp" | wc -l
...
```shell-session
Fr1zzleFry@htb[/htb]$ cat tnet.minrate300 | grep "/tcp" | wc -l
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.0/24 -F -oN tnet.T5 -T 5
...
```shell-session
Fr1zzleFry@htb[/htb]$ cat tnet.T5 | grep "/tcp" | wc -l
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p 21,22,25 -sS -Pn -n --disable-arp-ping --packet-trace
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p 21,22,25 -sA -Pn -n --disable-arp-ping --packet-trace
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p 80 -sS -Pn -n --disable-arp-ping --packet-trace -D RND:5
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -n -Pn -p445 -O
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -n -Pn -p 445 -O -S 10.129.2.200 -e tun0
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p50000 -sS -Pn -n --disable-arp-ping --packet-trace
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo nmap 10.129.2.28 -p50000 -sS -Pn -n --disable-arp-ping --packet-trace --source-port 53
...
```shell-session
Fr1zzleFry@htb[/htb]$ ncat -nv --source-port 53 10.129.2.28 50000
...
In the next three sections, we get different scenarios to practice where we have to scan our target. Firewall rules and IDS/IPS protect the systems, so we need to use the techniques shown to bypass the firewall rules and do
this as quiet as possible. Otherwise, we will be blocked by IPS.
...
2. nmap 10.129.2.80 -sV -F -D RND:5 --max-retries=0 -T4
