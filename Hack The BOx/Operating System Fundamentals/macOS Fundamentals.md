...
1. What BSD derivative is the basis of the macOS operating system?
	1. Darwin
2. What provides the desktop experience and file management capabilities within macOS?
	1. Finder
...
![[Pasted image 20231227183941.png]]

## Finder
![[Pasted image 20231227184006.png]]
...
## Spotlight
![[Pasted image 20231227184105.png]]
...
## Dock
![[Pasted image 20231227184150.png]]

## Launchpad
![[Pasted image 20231227184240.png]]
...
## Control Client
![[Pasted image 20231227184344.png]]
...
2. 2. Open the Mission Control app manually from the Launchpad.
...
|(`x`) - Execute|`Octal value of 1`|Basic File Permissions
#### Basic File Permission

```shell-session
Fr1zzleFry@htb[/htb]$ ls -l  
...
```shell-session
Fr1zzleFry@htb[/htb]$ chmod -vv 777 HTB-Wallpaper-1.png  
```
...
```shell-session
Fr1zzleFry@htb[/htb]$ sudo chown htb-student HTB-Wallpaper-1.png
```
...
![text](https://academy.hackthebox.com/storage/modules/157/system-info.png)
...
```shell-session
Fr1zzleFry@htb[/htb]$ ifconfig en0
```
...
```shell-session
Fr1zzleFry@htb[/htb]$ lsof -n -i4TCP -P    
...
```shell-session
Fr1zzleFry@htb[/htb]$ hostname
  
ACADEMY-MAC-2.local
```

```shell-session
Fr1zzleFry@htb[/htb]$ networksetup -listallnetworkservices  
...
```shell-session
Fr1zzleFry@htb[/htb]$ networksetup -listnetworkserviceorder  
...
```shell-session
Fr1zzleFry@htb[/htb]$ networksetup -getinfo Wi-Fi  
...
```shell-session
Fr1zzleFry@htb[/htb]$ networkQuality -I <interface> 
```
...
```shell-session
Fr1zzleFry@htb[/htb]$ security find-generic-password -wa Office-2.4G
Sup3r$ecure  
```
...
```shell-session
Fr1zzleFry@htb[/htb]$ brew search firefox
==> Formulae
firefoxpwa
...
In recent macOS releases, Keychain also allows us to store our two-factor authentication (2FA) keys with our passwords. It can automatically fill in the code when logging into the app, which makes it very convenient to be secure by enabling 2FA without any extra complications: ![macos_passwords](https://academy.hackthebox.com/storage/modules/157/macos_passwords.jpg)
...
```shell-session
Fr1zzleFry@htb[/htb]$ chsh -s /bin/bash
```

If you are on an older version of macOS and would like to install ZSH, you can do so with the following command:

```shell-session
Fr1zzleFry@htb[/htb]$ brew install zsh
Fr1zzleFry@htb[/htb]$ chsh -s /bin/zsh
```

While the main benefit of ZSH is its extensions, you may still find it more user-friendly than bash. For example, whenever we click on `tab` after a command to show possible options/arguments, ZSH shows all of them below our command and allows us to navigate between them by clicking `tab`: ![macos_zsh_tab](https://academy.hackthebox.com/storage/modules/157/macos_zsh_tab.jpg)
...
![notes](https://academy.hackthebox.com/storage/modules/157/macos_notes.png)
...
The most powerful feature of Alfred is its [workflows](https://www.alfredapp.com/workflows/), which perform specific actions through the search bar and is an ideal way for macOS automation. Alfred provides a free store for all open-source workflows provided by the community, in which we can find many workflows that can automate many of the tasks we need or make it easier to perform other tasks: ![workflows](https://academy.hackthebox.com/storage/modules/157/to-do-list-workflow.png)
